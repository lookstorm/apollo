...
public class NamespaceService {
    ...
    public List<Namespace> findNamespacesLike(String appId, String clusterName, String namespaceName, String keyName, int page, int size) {
        ...
        if (StringUtils.isBlank(namespaceName) && StringUtils.isBlank(keyName)) {
            namespaces = namespaceRepository.findByAppIdAndClusterNameOrderByIdAscLike(appId, clusterName, pageable).getContent();
        } else if (!StringUtils.isBlank(namespaceName) && StringUtils.isEmpty(keyName)) {
            namespaces = namespaceRepository.findByAppIdAndClusterNameOrderByIdAscLikeWithNamespace(appId, clusterName, namespaceName, pageable).getContent();
        } else if (StringUtils.isBlank(namespaceName) && !StringUtils.isBlank(keyName)) {
            namespaces = namespaceRepository.findByAppIdAndClusterNameOrderByIdAscLikeWithKey(appId, clusterName, keyName, pageable).getContent();
        } else if (!StringUtils.isBlank(namespaceName) && !StringUtils.isBlank(keyName)) {
            namespaces = namespaceRepository.findByAppIdAndClusterNameOrderByIdAscLikeWithAll(appId, clusterName, namespaceName, keyName, pageable).getContent();
        }
        if (namespaces == null) {
            return Collections.emptyList();
        }
